...
- 游戏内点歌、暂停、切歌、队列管理，支持 单曲/随机/顺序/循环 四种播放模式
- 搜索匹配歌曲名、艺术家、歌曲列表索引，无需打全称即可 播放/加入到播放队列
- 自动播放下一首歌，解放双手
- 管理员可在游戏内实时增删改歌曲信息
...
# 模板生成模式：从数据包文件夹生成歌名-艺术家模板
python midiplayer.pyz -g <数据包目录> [输出路径]
```
...
| `-g`/`--generate-template`（可选） | 从数据包文件夹生成歌名-艺术家模板文件 |
...
# 生成模板文件（从数据包文件夹自动提取 link）
python midiplayer.pyz -g ./datapacks/ ./output
# 会生成 song_template.txt，每行为 "link - 未知"
# 编辑模板文件后，再使用标准模式生成 songs.json
```
